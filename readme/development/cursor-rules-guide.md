...
| **`01.crud-rules.mdc`**            | Best practices for CRUD operations, especially Create vs Update field handling. *Essential for all data operations.* |
...
| **`03.create-next-js-component.mdc`** | **NEW:** Proactive component creation with proper separation of concerns. *Essential for creating any new complex component.* |
| **`03.initialize-new-sdk.mdc`**    | **NEW:** SDK client initialization patterns for environment variable dependent clients. *Essential for integrating any new SDK/API.* |

### **`98. One-Time Operations & Guides`** (Specific Tasks and Knowledge)

| Rule Filename                             | Purpose/When to Consult                                                                                                |
| :---------------------------------------- | :--------------------------------------------------------------------------------------------------------------------- |
| **`98.create-rules.mdc`**        | How to create, structure, and update Cursor Rules. *Consult before adding or modifying any rule.*                   |
| **`98.create-readme.mdc`**           | Template and guidelines for documenting functions with a README. *Use after creating/refactoring a function.*        |
| **`98.split-large-files.mdc`**       | Process for breaking down large, monolithic files. *For one-time refactoring of "god files."*                        |
| **`98.fix-build-errors.mdc`**               | **CRITICAL:** Methodology for fixing build errors without ESLint disables or shortcuts. *Essential for all error resolution.* |
| **`98.troubleshoot.mdc`**            | Collection of common issues and solutions (e.g., Next.js 15 async route params). *Check here first for errors.*      |
...
| :--------------------------------- | :---------------------------------------------------- |
| **Fixing Build/TypeScript/ESLint Errors** | **`98.fix-build-errors.mdc`** *(CRITICAL - Read First!)* |
| **Creating New Components**        | **`03.create-next-js-component.mdc`** *(Prevents future split needs!)* |
| **Integrating New SDK/API**        | **`03.initialize-new-sdk.mdc`** *(Prevents environment variable build errors!)* |
